...
async function startXeonBotInc() {
    const XeonBotInc = XeonBotIncConnect({
        ...
        browser: ['Folg mheg','Safari','1.0.0'],
        auth: state
    })
    ...
    XeonBotInc.sendMessage(callerId, { text: `Automatisches Sperrsystem!\nRufen Sie den Bot nicht an!\nBitte fragen oder kontaktieren Sie den Eigentümer, um Sie zu entsperren!`}, { quoted : xeonfek })
    await sleep(8000)
    ...
    // Get Profile Picture Group
       ...
       XeonBotInc.send5ButImg(pea[0].id, `「 Gruppeneinstellungen geändert 」\n\nDie Gruppe wurde vom Administrator geschlossen, jetzt kann nur der Administrator Nachrichten senden !`, `${botname}`, lolXeon, [])
       } else if(pea[0].announce == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Gruppeneinstellungen geändert 」\n\nDie Gruppe wurde vom Administrator geöffnet, jetzt können die Teilnehmer Nachrichten senden!`, `${botname}`, lolXeon, [])
       } else if (pea[0].restrict == true) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Gruppeneinstellungen geändert 」\n\nGruppeninformationen wurden eingeschränkt, jetzt kann nur der Administrator Gruppeninformationen bearbeiten !`, `${botname}`, lolXeon, [])
       } else if (pea[0].restrict == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Gruppeneinstellungen geändert 」\n\nGruppeninformationen wurden geöffnet, jetzt können die Teilnehmer Gruppeninformationen bearbeiten !`, `${botname}`, lolXeon, [])
       } else {
       XeonBotInc.send5ButImg(pea[0].id, `「 Gruppeneinstellungen geändert 」\n\nGruppenbetreff wurde geändert in *${pea[0].subject}*`, `${botname}`, lolXeon, [])
     }
    })
...
title: `${ownername}`,
body: `Vergessen Sie nicht, die Gruppenbeschreibung zu lesen`,
mediaType:2,
thumbnail: XeonWlcm,
sourceUrl: `${websitex}`,
mediaUrl: `${websitex}`
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   //you fail to put, i sue you for sure!
let buttons = [
{buttonId: `wkwkwk`, buttonText: {displayText: 'Tschüss'}, type: 1}
]
let buttonMessage = {
document: fs.readFileSync('./XeonMedia/theme/cheems.xlsx'),
mimetype: docs,
...
contextInfo:{externalAdReply:{
title: `${ownername}`,
body: `Wiedersehen! mein Freund, pass auf dich auf.`,
mediaType:2,
thumbnail: XeonLft,
sourceUrl: `${websitex}`,
mediaUrl: `${websitex}`
...
XeonBotInc.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
    ...
    XeonBotInc.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Fehlerhafte Sitzungsdatei, bitte Sitzung löschen und erneut scannen`); XeonBotInc.logout(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("Verbindung geschlossen, Verbindung neu...."); startXeonBotInc(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Verbindung zum Server unterbrochen, Verbindung neu..."); startXeonBotInc(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Verbindung ersetzt, weitere neue Sitzung geöffnet, bitte zuerst aktuelle Sitzung schließen"); XeonBotInc.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Gerät abgemeldet, bitte erneut scannen und ausführen.`); XeonBotInc.logout(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Neustart erforderlich, Neustart..."); startXeonBotInc(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Zeitüberschreitung bei Verbindung, Verbindung neu..."); startXeonBotInc(); }
            else XeonBotInc.end(`Unbekannter Trennungsgrund: ${reason}|${connection}`)
        }
